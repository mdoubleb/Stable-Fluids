 ...
 * Solve matrices with Smooth Plastic Boundary conditions and with scalar solving
 */
public class ComputeMatricesMTJ_BC2_Scalar {
	MTJSparseMatrix DiffusionX, Laplacien, DiffusionY, DiffusionScalar;
	float dx,dy;
	int N;
	float dt;
	float visc;
	...
	public ComputeMatricesMTJ_BC2_Scalar(int n, float dx, float dy, float dt, float visc, float kappa){
		N=n*n;
		this.dx=dx;
		this.dy=dy;
		this.dt=dt;
		...
		CompRowMatrix DScalar=new CompRowMatrix(N, N, mA);
		
		double Cx = -visc*dt/(dx*dx);
		double Cy = -visc*dt/(dy*dy);
		double CxScalar = -kappa*dt/(dx*dx);
		double CyScalar = -kappa*dt/(dy*dy);
		//Compute DX and DXScalar
		for(int i = 0; i<N;i++) {
			int k = i/n;
			int l = i%n;
			...
			DScalar.set(i,i,1-2*CxScalar -2*CyScalar);
			
			if(l!=0){
				DX.set(i,i-1,Cx);
				DScalar.set(i,i-1,CxScalar);
			}
			else{
				DX.set(i,i+n-1,0.);
				DScalar.set(i,i+n-1,0.);
			}
			if(l!=n-1){
				DX.set(i,i+1,Cx);
				DScalar.set(i,i+1,CxScalar);
			...
			if(k!=0){
				DX.set(i,i-n,Cy);
				DScalar.set(i,i-n,CyScalar);
			}
			else{
				DX.set(i,N-n+l,0.);
				DScalar.set(i,N-n+l,0.);
			}
			if(k!=n-1){
				DX.set(i,i+n,Cy);
				DScalar.set(i,i+n, CyScalar);
		...
		DiffusionScalar = new MTJSparseMatrix(DScalar);
	}
